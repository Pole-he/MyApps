...
package com.nathan.myapps.widget.gridview ;
...
public class StaggeredGridView extends ViewGroup {
         ...
         * modifications to StaggeredGridView. Like ListView, SGV populates from
         * an adapter and recycles views that fall out of the visible boundaries
         * of the grid. A few invariants always hold: - mFirstPosition is the
         * adapter position of the View returned by getChildAt(0). - Any child
         * index can be translated to an adapter position by adding
         * mFirstPosition. - Any adapter position can be translated to a child
         * index by subtracting mFirstPosition. - Views for items in the range
         * [mFirstPosition, mFirstPosition + getChildCount()) are currently
         * attached to the grid as children. All other adapter positions do not
         * have active views. This means a few things thanks to the staggered
         * grid's nature. Some views may stay attached long after they have
         * scrolled offscreen if removing and recycling them would result in
         * breaking one of the invariants above. LayoutRecords are used to track
         * data about a particular item's layout after the associated view has
         * been removed. These let positioning and the choice of column for an
         * item remain consistent even though the rules for filling content up
         * vs. filling down vary. Whenever layout parameters for a known
         * LayoutRecord change, other LayoutRecords before or after it may need
         * to be invalidated. e.g. if the item's height or the number of columns
         * it spans changes, all bets for other items in the same direction are
         * off since the cached information no longer applies.
         */
        ...
        /**
         * The listener that receives notifications when an item is long
         * clicked.
         */
        ...
        private static final class LayoutRecord {
                ...
                private final void ensureMargins ( ) {
                        if ( mMargins == null ) {
                                ...
                                // all layoutrecords are purged when column
                                // count changes.
        ...
        public StaggeredGridView ( Context context , AttributeSet attrs , int defStyle ) {
                ...
                if ( attrs != null ) {
                        ...
                        mDrawSelectorOnTop = a.getBoolean ( R.styleable.StaggeredGridView_drawSelectorOnTop_gv , false ) ;
                }
                else {
                        mColCount = 2 ;
        ...
        /**
         * Set a fixed number of columns for this grid. Space will be divided
         * evenly among all columns, respecting the item margin between columns.
         ...
         * 
         * @return the adapter position represented by the view at
         *         getChildAt(0).
         */
        ...
        public boolean onTouchEvent ( MotionEvent ev ) {
                ...
                case MotionEvent.ACTION_MOVE :
                        ...
                        if ( mTouchMode == TOUCH_MODE_DRAGGING ) {
                                ...
                                if ( ! trackMotionScroll ( deltaY , true ) ) {
                                        // Break fling velocity if we impacted
                                        // an edge.
                                        mVelocityTracker.clear ( ) ;
                                }
                        }
                ...
                case MotionEvent.ACTION_UP : {
                        ...
                        final float velocity = VelocityTrackerCompat
        ...
        private void recycleOffscreenViews ( ) {
                ...
                for ( int i = getChildCount ( ) - 1 ; i >= 0 ; i -- ) {
                        ...
                        if ( child.getTop ( ) <= clearBelow ) {
                                // There may be other offscreen views, but we
                                // need to maintain
                ...
                if ( childCount > 0 ) {
                        // Repair the top and bottom column boundaries from the
                        // views we still have
        ...
        private void populate ( boolean clearData ) {
                ...
                for ( int i = 0 ; i < colCount ; i ++ ) {
                        final int offset = top
                                        + ( ( mRestoreOffsets != null ) ? Math.min ( mRestoreOffsets [ i ] , 0 ) : 0 ) ;
        ...
        final void layoutChildren ( boolean queryAdapter ) {
                ...
                final int colWidth = ( getWidth ( ) - paddingLeft - paddingRight - itemMargin * ( mColCount - 1 ) )
                                / mColCount ;
                mColWidth = colWidth ;
                ...
                for ( int i = 0 ; i < childCount ; i ++ ) {
                        ...
                        if ( queryAdapter ) {
                                ...
                                lp = ( LayoutParams ) child.getLayoutParams ( ) ; // Might
                                                                                  // have
                                                                                  // changed
                        }
                        ...
                        if ( span > 1 ) {
                                ...
                                // final int colEnd = Math.min(mColCount, col +
                                // lp.span);
                        ...
                        if ( rec != null && rec.height != childHeight ) {
                                // Invalidate our layout records for everything
                                // before this.
         ...
         * @param overhang
         *                the number of extra pixels to fill beyond the current
         *                top edge
         * @return the max overhang beyond the beginning of the view of any
        ...
        final int fillUp ( int fromPosition , int overhang ) {
                ...
                while ( nextCol >= 0 && mItemTops [ nextCol ] > fillTo && position >= 0 ) {
                        // make sure the nextCol is correct. check to see if has
                        // been mapped
                ...
                if ( this.getChildCount ( ) > column ) {
                        for ( int i = 0 ; i < this.mColCount ; i ++ ) {
                                ...
                                if ( child != null ) {
                                        ...
                                        // determine the column by cycling
                                        // widths
         ...
         *                the number of extra pixels to fill beyond the current
         *                bottom edge
         * @return the max overhang beyond the end of the view of any added
         *         items at the bottom
         */
        final int fillDown ( int fromPosition , int overhang ) {
                ...
                while ( nextCol >= 0 && mItemBottoms [ nextCol ] < fillTo && position < mItemCount ) {
                         ...
                         */
                        // child.measure(MeasureSpec.EXACTLY,
                        ...
                        if ( span > 1 ) {
                                ...
                                // final int colEnd = Math.min(mColCount,
                                // nextCol + lp.span);
                        ...
                        if ( ! mColMappings.get ( nextCol ).contains ( position ) ) {
                                ...
                                // check to see if the mapping exists in other
                                // columns
        ...
        /**
         * @return column that the next view filling upwards should occupy. This
         *         is the bottom-most position available for a single-column
         *         item.
         */
        ...
        /**
         * @return column that the next view filling downwards should occupy.
         *         This is the top-most position available.
         */
        ...
        /**
         * Obtain a populated view from the adapter. If optScrap is non-null and
         * is not reused it will be placed in the recycle bin.
         * 
         * @param position
        ...
        public void setAdapter ( ListAdapter adapter ) {
                ...
                // remove certain layout records
                // and onscreen views if they have changed instead of removing
                // all of the state here.
                clearAllState ( ) ;
        ...
        /**
         * Clear all state because the grid will be used for a completely
        ...
        private void clearAllState ( ) {
                ...
                // Clear recycler because there could be different view types
                // now
                mRecycler.clear ( ) ;
        ...
        /**
         * Scroll the list so the first visible position in the grid is the
         * first item in the adapter.
         */
        public void setSelectionToTop ( ) {
                // Clear out the views (but don't clear out the layout records
                // or recycler because the data
                // has not changed)
                removeAllViews ( ) ;
        ...
        private class AdapterDataSetObserver extends DataSetObserver {
                ...
                public void onChanged ( ) {
                        ...
                        if ( lazyload ) {
                                if ( lastCount != mItemCount ) {
                                        ...
                                        if ( childCount > 0 ) {
                                                ...
                                                for ( int i = 0 ; i < childCount ; i ++ ) {
                                                        ...
                                                        final int top = child.getTop ( ) - mItemMargin ;
                                                        final int bottom = child.getBottom ( ) ;
                                                        final LayoutRecord rec = mLayoutRecords.get ( mFirstPosition
                                                                        + i ) ;
                                ...
                                // TODO: Consider matching these back up if we
                                // have stable IDs.
                                mRecycler.clearTransientViews ( ) ;
                                
                                if ( ! mHasStableIds ) {
                                        // Clear all layout records and recycle
                                        // the views
                                        mLayoutRecords.clear ( ) ;
                                        recycleAllViews ( ) ;
                                        
                                        // Reset item bottoms to be equal to
                                        // item tops
                                        final int colCount = mColCount ;
                                        for ( int i = 0 ; i < colCount ; i ++ ) {
                                                mItemBottoms [ i ] = mItemTops [ i ] ;
                                ...
                                // reset list if position does not exist or id
                                // for position has changed
                        ...
                        // TODO: consider repopulating in a deferred runnable
                        // instead
        ...
        static class SavedState extends BaseSavedState {
                ...
                @ Override
                public String toString ( ) {
                        return "StaggereGridView.SavedState{" + Integer.toHexString ( System.identityHashCode ( this ) )
        ...
        /**
         * The top-level view of a list item can implement this interface to
         * allow itself to modify the bounds of the selection shown for that
         * item.
         */
        public interface SelectionBoundsAdjuster {
                 ...
                 *                selected for the item (that is the bounds of
                 *                the entire view). The values can be modified
                 *                as desired.
                 */
        ...
        protected int [ ] onCreateDrawableState ( int extraSpace ) {
                ...
                // The selector uses this View's drawable state. The selected
                // child view
                // is disabled, so we need to remove the enabled state from the
                // drawable
                // states.
                final int enabledState = ENABLED_STATE_SET [ 0 ] ;
                
                // If we don't have any extra space, it will return one of the
                // static state arrays,
                // and clearing the enabled state on those arrays is a bad
                // thing! If we specify
                // we need extra space, it will create+copy into a new array
                // that safely mutable.
        ...
        private class PerformClick extends WindowRunnnable implements Runnable {
                ...
                public void run ( ) {
                        // The data has changed since we posted this action in
                        // the event queue,
                        ...
                        if ( adapter != null && mItemCount > 0 && motionPosition != INVALID_POSITION
                                ...
                                // If there is no view, something bad happened
                                // (the view scrolled off the
        ...
        boolean performLongPress ( final View child , final int longPressPosition , final long longPressId ) {
                ...
                // TODO : add check for multiple choice mode.. currently modes
         ...
         * {@link #getContextMenuInfo()}. This methods knows the view, position
        ...
        public static class AdapterContextMenuInfo implements ContextMenuInfo {
                 ...
                 * This will be one of the children of this AdapterView.
                 */
                public View targetView ;
                
                /**
                 * The position in the adapter for which the context menu is
                 * being displayed.
                 */
                public int position ;
                
                /**
                 * The row id of the item for which the context menu is being
                 * displayed.
                 */
                public long id ;
        }
        ...
        /**
         * Returns the selector {@link android.graphics.drawable.Drawable} that
         * is used to draw the selection in the list.
         * 
        ...
        /**
         * Set a Drawable that should be used to highlight the currently
         * selected item.
         * 
         * @param resID
        ...
        /**
         * Indicates whether this view is in a state where the selector should
         * be drawn. This will happen if we have focus but are not in touch
         * mode, or we are in the middle of displaying the pressed state for an
         * item.
         * 
        ...
        /**
         * @return True if the current touch mode requires that we draw the
         *         selector in the pressed state.
         */
        ...
        /**
         * @return The callback to be invoked with an item in this AdapterView
        ...
        public interface OnItemClickListener {
                 ...
                 *                clicked (this will be a view provided by the
                 *                adapter)
                 * @param position
        ...
        /**
         * @return The callback to be invoked with an item in this AdapterView
         *         has been clicked and held, or null id no callback as been
         *         set.
         */
        ...
        public interface OnItemLongClickListener {
                ...
                /**
                 * Callback method to be invoked when an item in this view has
                 ...
                 * @param view
                 *                The view within the AbsListView that was
                 *                clicked
                 * @param position
         ...
         *         or {@link #INVALID_POSITION} if the point does not intersect
         *         an item.
         */
        ...
        public void setHeaderView ( View v ) {
                ...
                StaggeredGridView.LayoutParams lp = new StaggeredGridView.LayoutParams (
