...
public class StaggeredGridView extends ViewGroup {
     ...
     * position of the View returned by getChildAt(0). - Any child index can be
     * translated to an adapter position by adding mFirstPosition. - Any adapter
     * position can be translated to a child index by subtracting
     * mFirstPosition. - Views for items in the range [mFirstPosition,
     * mFirstPosition + getChildCount()) are currently attached to the grid as
     * children. All other adapter positions do not have active views. This
     * means a few things thanks to the staggered grid's nature. Some views may
     * stay attached long after they have scrolled offscreen if removing and
     ...
     * filling content up vs. filling down vary. Whenever layout parameters for
     * a known LayoutRecord change, other LayoutRecords before or after it may
     * need to be invalidated. e.g. if the item's height or the number of
     * columns it spans changes, all bets for other items in the same direction
     * are off since the cached information no longer applies.
     */
    ...
    public StaggeredGridView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            TypedArray a = getContext()
                    .obtainStyledAttributes(attrs, R.styleable.StaggeredGridView);
            mColCount = a.getInteger(R.styleable.StaggeredGridView_numColumns, 2);
            mDrawSelectorOnTop = a.getBoolean(R.styleable.StaggeredGridView_drawSelectorOnTop_gv,
                    false);
        }
        else {
            mColCount = 2;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_UP: {
            ...
            if (Math.abs(velocity) > mFlingVelocity) { // TODO
                ...
                mScroller
                        .fling(0, 0, 0, (int) velocity, 0, 0, Integer.MIN_VALUE, Integer.MAX_VALUE);
                mLastTouchY = 0;
                invalidate();
            }
            else {
    ...
    public int getmNumCols() {
        return mNumCols;
    }

    public void setmNumCols(int mNumCols) {
