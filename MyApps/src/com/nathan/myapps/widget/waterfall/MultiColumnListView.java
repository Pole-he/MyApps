...
import com.nathan.myapps.BuildConfig;
...
public class MultiColumnListView extends PLA_ListView {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs == null) {
            ...
            int landColNumber = a.getInteger(
                    R.styleable.PinterestLikeAdapterView_plaLandscapeColumnNumber, -1);
            int defColNumber = a.getInteger(R.styleable.PinterestLikeAdapterView_plaColumnNumber,
                    -1);
            ...
            mColumnPaddingLeft = a.getDimensionPixelSize(
                    R.styleable.PinterestLikeAdapterView_plaColumnPaddingLeft, 0);
            mColumnPaddingRight = a.getDimensionPixelSize(
                    R.styleable.PinterestLikeAdapterView_plaColumnPaddingRight, 0);
            a.recycle();
        }
    ...
    private Column gettBottomColumn() {
        ...
        if (DEBUG)
            Logger.d("Column", "get Shortest Bottom Column: " + result.getIndex());
        return result;
    }
        ...
        if (listener != null) {
            this.setOnScrollListener(new OnScrollListener()
            {
                ...
                public void onScrollStateChanged(PLA_AbsListView view, int scrollState) {
                    // FIXME 有时判断不准确
                    int firstVisiblePosition = getFirstVisiblePosition();// 屏幕上第一项在Adapter中的position
                    int childCount = getChildCount();// 屏幕上显示的数据项的总数
                    int lastIndex = getAdapter().getCount() - OFFSET;
                    if (scrollState == OnScrollListener.SCROLL_STATE_IDLE
                            && firstVisiblePosition + childCount >= lastIndex
                            && loadingMoreComplete) {
                ...
                public void onScroll(PLA_AbsListView view, int firstVisibleItem,
                    ...
                    if (BuildConfig.DEBUG) {
                        Logger.e(MultiColumnListView.class.getSimpleName(), "the first index is "
                                + firstItemIndex);
                    }
                }
            });
